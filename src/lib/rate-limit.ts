...
export class RateLimit {
    ...
    public async last5MinutesTxs(walletAddress: string) {
        ...
        const signatures = await connection.getSignaturesForAddress(new PublicKey(walletAddress), { limit: MAX_5_MIN_TXS_ALLOWED });
      ...
      if (elapsedTime >= 1) {
          ...
          if (tps >= MAX_TPS_ALLOWED) {
              ...
              setTimeout(() => {
                  ...
                  for (const user of wallet.userWallets) {
                      bot.sendMessage(user.userId, this.rateLimitMessages.walletWasResumed(wallet.address), { parse_mode: 'HTML' });
                  }
