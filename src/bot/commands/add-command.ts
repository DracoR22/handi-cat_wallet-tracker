...
export class AddCommand {
    ...
    private add({ message, isButton }: { message: TelegramBot.Message, isButton: boolean }) {
        ...
        const listener = async (responseMsg: TelegramBot.Message) => {
            ...
            const walletEntries = text?.split('\n').map(entry => entry.trim()).filter(Boolean); // Split input by new lines, trim, and remove empty lines
            ...
            for (const entry of walletEntries) {
              ...
              if (latestWalletTxs && latestWalletTxs >= MAX_5_MIN_TXS_ALLOWED) {
                this.bot.sendMessage(message.chat.id, `😾 Wallet ${walletAddress} is spamming too many transactions, try another wallet or try again later`);
                continue;
              }
